...
function testSkipWhile(t, [from, to], skipUntil) {
    ...
    // Kirka skip
    const spyFilter = useSpy((item) => skipUntil(item));
    ...
    const shouldBeCalledAmount = Math.min(values.length - skippedValues.length + 1, values.length);

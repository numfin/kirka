...
export namespace M {
  ...
  >;
  export class MaybeConstructor<T, S extends MaybeState = MaybeState>
    implements AsyncMonad<T>, Alternative<T>, Container<T>
  {
    ...
    static none<T = never>(): Maybe<T> {
      return new MaybeConstructor<T, MaybeState.None>(
        MaybeState.None,
        undefined
      );
    }
  ...
  export const { merge, just, none, from, fromNullable, chain } =
    MaybeConstructor;
